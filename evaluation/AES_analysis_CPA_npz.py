...
Reads CSV from the averaging capture with columns:
...
Also plots the correlation curve for byte 0:
- best guess (max |corr|)
- one wrong guess (auto-picked)
"""
...
csv_file         = data_dir_path / "trace_overview.csv"

max_blocks    = 10_000           # None = use all rows
...
def main():
    ...
    for byte_index in range(16):
        ...
        if byte_index == 0 and corr_vec_best is not None:
            ...
            # pick a wrong guess: 0x00 unless it's equal to best; else 0x01
    ...
    if SHOW_CORR_PLOT and corr_vec_best_b0 is not None:
        ...
        if corr_vec_wrong_b0 is not None:
            plt.plot(x, corr_vec_wrong_b0, label=f"Byte0 wrong guess 0x{wrong_guess_b0:02X}", alpha=0.85)

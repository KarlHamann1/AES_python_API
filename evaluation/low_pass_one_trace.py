...
- Save the filtered trace next to the input with a descriptive filename.
"""
...
def _design_lowpass_sos(fs: float, f_cut: float, order: int):
    """Butterworth LPF (SOS). Clamp edges if needed."""
    nyq = fs * 0.5
    f_cut = max(1.0, min(f_cut, nyq * 0.999))  # keep inside (0, Nyquist)
    return sps.butter(order, f_cut, btype="low", output="sos", fs=fs)
...
def _plot_response(sos, fs: float, cutoff_hz: float, order: int, save_to: Path | None):
    ...
    plt.semilogx(w, mag_db, label="Low-pass response")
    plt.title(f"Butterworth LPF (order={order}) — cutoff ~ {label_cut}")
...
def main():
    ...
    # design + response
    sos = _design_lowpass_sos(SAMPLING_RATE, LOW_PASS_CUTOFF_HZ, FILTER_ORDER)
    resp_png = in_path.with_suffix("").with_name(
        in_path.stem + f"__lpf_{int(LOW_PASS_CUTOFF_HZ)}Hz__resp.png"
    ) if SAVE_RESP_PNG else None
    _plot_response(sos, SAMPLING_RATE, LOW_PASS_CUTOFF_HZ, FILTER_ORDER, resp_png)
    ...
    # filter (zero-phase); retry with smaller pad if the vector is short
    try:
    ...
    # save next to input
    label_cut = (f"{LOW_PASS_CUTOFF_HZ/1e3:.0f}k" if LOW_PASS_CUTOFF_HZ < 1e6
                else f"{LOW_PASS_CUTOFF_HZ/1e6:.1f}M")
    out_name  = in_path.stem + f"__lpf_{label_cut}.npy"
    ...
    plt.plot(filtered, label=f"Low-pass ({label_cut})")
    plt.title("Filtered Trace (Low-pass)")

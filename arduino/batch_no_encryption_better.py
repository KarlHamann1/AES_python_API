#!/usr/bin/env python3
"""
Baseline (noise‑only) trace capture.
No AES is triggered – the PicoScope fires via auto‑trigger after the specified
timeout.  Useful for building a reference noise set for filtering/SNR studies.
"""

import os, sys, csv, time, secrets     # secrets only to mimic plaintext bytes
from pathlib import Path
import numpy as np
from tqdm import trange

ROOT = Path(__file__).resolve().parent
sys.path.append(str(ROOT.parent))

from arduino.encipher import EncipherAPI
from picoscope_acquisition import DataAcquisition


def batch_no_encryption(
    num_rounds      = 10_000,
    duration        = 0.010,         # 10 ms window
    filename_prefix = "noise",
    output_dir      = "data_arduino_noise",
    port            = "COM5",
    baud            = 9600
):
    """Capture *num_rounds* auto‑triggered traces with no AES activity."""

    # UART is kept alive only so the MCU remains powered/clocked
    enc = EncipherAPI(port=port, baudrate=baud, timeout=1)

    scope = DataAcquisition(
        device_resolution_bits = 12,
        timebase               = 43,           # ~1.56 MS/s (same as AES run)
        sampling_rate_hz       = 1.56e6,
        capture_duration_s     = duration,
        capture_channel        = "A",
        trigger_channel        = "B",          # still the same channel, but no edge expected
        trigger_threshold_mV   = 100,
        trigger_delay_samples  = 0,
        auto_trigger_ms        = 1000,         # fires after 1 s if no trigger edge
        coupling_mode          = "AC",
        voltage_range          = "10MV",
        output_dir             = output_dir,
        filename_prefix        = filename_prefix
    )

    Path(output_dir).mkdir(parents=True, exist_ok=True)
    csv_path = Path(output_dir) / "trace_overview.csv"

    try:
        scope.setup_picoscope()

        with open(csv_path, "w", newline="") as csv_f:
            writer = csv.writer(csv_f)
            writer.writerow(
                ["TraceIndex", "DummyPlaintextHex",
                "FileName",   "Samples"]
            )

            for idx in trange(num_rounds, desc="Capturing (noise)"):
                # 0  Dummy plaintext for consistency (never used)
                dummy_pt = secrets.token_bytes(16)
                pt_hex = dummy_pt.hex().upper()

                # 1  Send state (not needed but keeps protocol identical)
                enc.set_state(dummy_pt)

                # 2  Arm the scope – will wait for real trigger (none) or auto‑trigger
                scope.prepare_block_mode()

                # 3  DO NOT call enc.encrypt(); simply wait for block to complete
                wf = scope.capture_block()
                if wf is None or len(wf) == 0:
                    print(f"[Warn] auto‑trigger timeout – empty capture")
                    continue

                # 4  Save NPZ
                fname = f"{filename_prefix}_{idx:06d}.npz"
                np.savez(
                    Path(output_dir) / fname,
                    trace      = wf.astype(np.float32),
                    plaintext  = np.frombuffer(dummy_pt, np.uint8),
                    noise_only = True,
                    sr         = scope.sampling_rate_hz,
                    timebase   = scope.timebase_final
                )

                # 5  Log CSV
                writer.writerow([idx, pt_hex, fname, len(wf)])
                if idx % 500 == 0:
                    csv_f.flush()

        print(f"\nFinished. Overview CSV: {csv_path}")

    finally:
        scope.close()
        enc.close()


if __name__ == "__main__":
    batch_no_encryption()
